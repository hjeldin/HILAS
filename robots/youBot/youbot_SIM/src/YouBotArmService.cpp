...
YouBotArmService::YouBotArmService(const string& name, TaskContext* parent, long clientID):
Hilas::IRobotArmService(name, parent, YouBot::NR_OF_ARM_SLAVES, 15, YouBot::JOINT_ARM_NAME_ARRAY,clientID), m_min_slave_nr(1)
{
    ...
    {
      simxGetObjectHandle(m_clientID, YouBot::JOINT_ARM_NAME_ARRAY[i].c_str(), &vrep_joint_handle[i], simx_opmode_oneshot_wait);
    }
