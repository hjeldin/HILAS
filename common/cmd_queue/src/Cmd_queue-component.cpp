#include "Cmd_queue-component.hpp"
#include <rtt/Component.hpp>
#include <iostream>

Cmd_queue::Cmd_queue(std::string const& name) : TaskContext(name)
{
	isinloading = true;
    ...
    this->addOperation("setIsInLoading", &Cmd_queue::setQueueMode, this);
	this->addProperty("isEmpty",isEmpty);

    //this->addPort("from_hw_odom", from_hw_odom).doc("Odometry status from HW");
    this->addPort("from_planner_goal_in", from_planner_goal).doc("Planner status from HW");    
   	this->addPort("from_cartesian_status_in", from_cartesian_status).doc("Cartesian status from HW"); 
    //this->addPort("from_hw_arm_joints", from_hw_arm_joints).doc("Arm joints status from HW");

    this->addPort("ros_arm_joint_position_command_in", ros_arm_joint_position_command).doc("ROS Command joint angles");
    this->addPort("ros_arm_joint_velocity_command_in", ros_arm_joint_velocity_command).doc("ROS Command joint velocities");
    this->addPort("ros_arm_joint_effort_command_in", ros_arm_joint_effort_command).doc("ROS Command joint torques");	

    this->addPort("ros_base_cmd_twist_in", ros_base_cmd_twist).doc("ROS Command base twist");
    this->addPort("ros_gripper_joint_position_command_in", ros_gripper_joint_position_command).doc("ROS Command the gripper position");    

    this->addPort("ros_planner_command_in", ros_planner_command).doc("ROS planner command");
    this->addPort("ros_cartesian_command_in", ros_cartesian_command).doc("ROS cartesian command");    

    this->addPort("orocos_arm_joint_position_command_in", orocos_arm_joint_position_command).doc("Orocos Command joint angles");
    this->addPort("orocos_arm_joint_velocity_command_in", orocos_arm_joint_velocity_command).doc("Orocos Command joint velocities");
    this->addPort("orocos_arm_joint_effort_command_in", orocos_arm_joint_effort_command).doc("Orocos Command joint torques");	

    this->addPort("orocos_base_cmd_twist_in", orocos_base_cmd_twist).doc("Orocos Command base twist");
    this->addPort("orocos_gripper_joint_position_command_in", orocos_gripper_joint_position_command).doc("Orocos Command the gripper position");    

    this->addPort("arm_joint_position_command_out", out_arm_joint_position_command).doc("Out command joint angles");
    this->addPort("arm_joint_velocity_command_out", out_arm_joint_velocity_command).doc("Out command joint velocities");
    this->addPort("arm_joint_effort_command_out", out_arm_joint_effort_command).doc("Out command joint torques");	

    this->addPort("base_cmd_twist_out", out_base_cmd_twist).doc("Out command base twist");
    this->addPort("gripper_joint_position_command_out", out_gripper_joint_position_command).doc("Out command the gripper position");

    this->addPort("ros_planner_command_out", out_ros_planner_command).doc("Out ROS planner command");
    this->addPort("ros_cartesian_command_out", out_ros_cartesian_command).doc("Out ROS cartesian command");    
}

void Cmd_queue::setQueueMode(bool mode)
{
	if(!mode)
	{
...
void Cmd_queue::my_push_front(const queue_item& q)
{
	if(queue.size() == 0)
	{
...
bool Cmd_queue::lastCartesianPoseIsReached()
{
...
bool Cmd_queue::configureHook()
{
	return true;
}

bool Cmd_queue::startHook()
{
	return true;
}

void Cmd_queue::updateHook()
{
...
void Cmd_queue::stopHook()
{

}

void Cmd_queue::cleanupHook()
{

}

ORO_CREATE_COMPONENT(Cmd_queue)
