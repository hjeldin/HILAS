...
void readTwistCallback(const geometry_msgs::TwistStamped::ConstPtr& msg)
{
  ...
  {
    odometry.header.frame_id = robot_name + "_tf/odom";
    odometry.child_frame_id = robot_name + "_tf/base_link";
    ...
    odometryTransform.header.frame_id = robot_name + "_tf/" + TF_ODOM_FRAME_ID;
    odometryTransform.child_frame_id = robot_name + "_tf/" + TF_ODOM_CHILD_FRAME_ID;
...
void pointCloud2LaserScanCallback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
    ...
    output.header.frame_id = robot_name + "_tf" + output_frame_id_; // Set output frame. Point clouds come from "optical" frame, scans come from corresponding mount frame
...
int main(int argc,char* argv[])
{
  ...
  ros::Subscriber subCmdPos = node.subscribe<motion_control_msgs::JointPositions>(robot_name + TOPIC_ARM_JOINT_POSITION_COMMAND, 1, cmdPosArmJointCallback);

  // Ros Subscriber Command Gripper Position
  ros::Subscriber subCmdGripperPos = node.subscribe<motion_control_msgs::JointPositions>(robot_name + TOPIC_GRIPPER_POSITION_COMMAND, 1, cmdPosGripperCallback);

  // Ros Subscriber Command Arm Velocity
  ros::Subscriber subCmdVel = node.subscribe<motion_control_msgs::JointVelocities>(robot_name + TOPIC_VELOCITY_COMMAND, 1, cmdVelCallback);

  // Ros Subscriber Command Arm Torque
  ros::Subscriber subCmdForce = node.subscribe<motion_control_msgs::JointEfforts>(robot_name + TOPIC_TORQUE_COMMAND, 1, cmdForceCallback);

  // Ros Subscriber Command Base Twist
  ros::Subscriber subCmdTwist = node.subscribe<geometry_msgs::Twist>(robot_name + TOPIC_TWIST_COMMAND, 1, cmdTwistCallback);

  // Ros Subscriber Twist (odom part)
  ros::Subscriber subReadTwist = node.subscribe<geometry_msgs::TwistStamped>(robot_name + TOPIC_TWIST_READ, 1, readTwistCallback);
  ...
  ros::Subscriber subJointStates = node.subscribe<sensor_msgs::JointState>(robot_name + TOPIC_ALL_JOINT_STATE, 1, allJointStateCallback);

  /* Ros Publisher JointStates remapped for Orocos */
  /* ARM */
  pubArmJointState = node.advertise<sensor_msgs::JointState>(robot_name + TOPIC_ARM_JOINT_STATE, 10);
  /* BASE */
  pubBaseJointState = node.advertise<sensor_msgs::JointState>(robot_name + TOPIC_BASE_JOINT_STATE, 10);


  // Ros Publisher Odom
  pubOdom = node.advertise<nav_msgs::Odometry>(robot_name + "/odom",10);

  laserScan = node.advertise<sensor_msgs::LaserScan>(robot_name + "/base_scan",10);
  ...
  */
  ...
  ros::Subscriber subArmJointStatesFromHW = node.subscribe<sensor_msgs::JointState>(robot_name + TOPIC_ARM_JOINT_STATE_FROM_HW, 1, armJointStateFromHWCallback);
  ros::Subscriber subBaseJointStatesFromHW = node.subscribe<sensor_msgs::JointState>(robot_name + TOPIC_BASE_JOINT_STATE_FROM_HW, 1, baseJointStateFromHWCallback);
  ros::Subscriber subOdometryFromHW = node.subscribe<nav_msgs::Odometry>(robot_name + TOPIC_ODOM_STATE_FROM_HW, 1, odomStateFromHWCallback);

  pubVisualizationMode = node.advertise<std_msgs::Int32>(robot_name + TOPIC_RESET_DYNAMIC,1);

  pubGeometryPoseToVrep = node.advertise<geometry_msgs::PoseStamped>(robot_name + TOPIC_POSE_STATE_TO_VREP,1);
